  ...
  async (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (error || !data) {
      return next(
        new ApiError(
          403,
          'Application profile not ready yet. Sign out, sign back in, and try again.'
        )
      );
    }
    ...
    if (FACULTY_ONLY_OPS_ROLES.includes(data.role as (typeof FACULTY_ONLY_OPS_ROLES)[number])) {
