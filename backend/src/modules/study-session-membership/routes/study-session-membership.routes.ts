...
import {
  createStudySessionMembershipSchema,
  listSessionMembersSchema,
  updateStudySessionMembershipSchema
} from '../dto/study-session-membership.dto.js';
...
router.post(
  '/join',
  validateRequest(createStudySessionMembershipSchema),
  asyncHandler((req, res) => studySessionMembershipWorkflowController.join(req, res))
);
router.get(
  '/session/:sessionId/members',
  validateRequest(listSessionMembersSchema),
  asyncHandler((req, res) => studySessionMembershipWorkflowController.listMembers(req, res))
);
