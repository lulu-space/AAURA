...
export const studySessionMembershipService = new SupabaseCrudService(
  'study_session_membership',
  '*',
  {
    ownerColumn: 'user_id',
...
export class StudySessionMembershipWorkflowService {
  async join(studySessionId: string, userId: string) {
  ...
  async listSessionMembers(studySessionId: string, actorUserId: string, actorRole?: string) {
    const { data: session, error: sessionError } = await supabaseAdmin
      .from('study_sessions')
    ...
    if (!isHost && !isAdmin) {
      ...
      if (mineError) throw new ApiError(500, 'Failed to verify membership.', mineError);
      if (!mine) {
    ...
    if (error) throw new ApiError(500, 'Failed to fetch session members.', error);
...
export const studySessionMembershipWorkflowService =
  new StudySessionMembershipWorkflowService();
