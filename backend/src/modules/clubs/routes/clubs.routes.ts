...
router.get(
  '/activity/feed',
  asyncHandler((req, res) => clubsWorkflowController.activityFeed(req, res))
);
router.get(
  '/:id/members',
  asyncHandler((req, res) => clubsWorkflowController.listMembers(req, res))
);
router.get(
  '/reports/monthly',
  authorizeCapability(CLUB_ORGANIZER_ROLES),
  asyncHandler((req, res) => clubsWorkflowController.monthlyReport(req, res))
);
router.post(
  '/system/detect-inactive',
  authorizeRoles('admin'),
  asyncHandler((req, res) => clubsWorkflowController.detectInactive(req, res))
);
router.patch(
  '/:id/reactivate',
  authorizeCapability(CLUB_ORGANIZER_ROLES),
  asyncHandler((req, res) => clubsWorkflowController.reactivate(req, res))
);

// Override list with member counts + next event before the generic CRUD list.
router.get(
  '/',
  asyncHandler((req, res) => clubsWorkflowController.listWithCounts(req, res))
);
