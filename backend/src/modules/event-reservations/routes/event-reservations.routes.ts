...
import {
  checkInSchema,
  joinEventSchema,
  listEventAttendeesSchema,
  previewEventJoinSchema,
  reserveEventSchema
} from '../dto/event-reservations-workflow.dto.js';

const router = Router();

router.post(
  '/reserve',
  validateRequest(reserveEventSchema),
  asyncHandler((req, res) => eventReservationsWorkflowController.reserve(req, res))
);
router.post(
  '/join',
  validateRequest(joinEventSchema),
  asyncHandler((req, res) => eventReservationsWorkflowController.join(req, res))
);
router.get(
  '/join/:token',
  validateRequest(previewEventJoinSchema),
  asyncHandler((req, res) => eventReservationsWorkflowController.previewJoin(req, res))
);
router.post(
  '/check-in',
  validateRequest(checkInSchema),
  asyncHandler((req, res) => eventReservationsWorkflowController.checkIn(req, res))
);
router.get('/mine', asyncHandler((req, res) => eventReservationsWorkflowController.listMine(req, res)));
router.get(
  '/event/:eventId/attendees',
  validateRequest(listEventAttendeesSchema),
  asyncHandler((req, res) => eventReservationsWorkflowController.listAttendees(req, res))
);
