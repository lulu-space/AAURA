...
const REVIEWER_ROLES = ['student_affairs', 'dean_of_faculty', 'admin'] as const;
...
router.get(
  ...
  asyncHandler((req, res) => eventsReviewController.listPending(req, res))
);
router.get(
  '/reviews/all',
  authorizeRoles(...REVIEWER_ROLES),
  asyncHandler((req, res) => eventsReviewController.listAll(req, res))
);
router.patch(
  '/:id/approve',
  authorizeRoles(...REVIEWER_ROLES),
  validateRequest(reviewEventSchema),
  asyncHandler((req, res) => eventsReviewController.approve(req, res))
);
router.patch(
  '/:id/reject',
  authorizeRoles(...REVIEWER_ROLES),
  validateRequest(reviewEventSchema),
  asyncHandler((req, res) => eventsReviewController.reject(req, res))
);
router.patch(
  '/:id/withdraw-approval',
  authorizeRoles(...REVIEWER_ROLES),
  validateRequest(reviewEventSchema),
  asyncHandler((req, res) => eventsReviewController.withdraw(req, res))
);

router.get(
  '/organizer/dashboard',
  authorizeCapability(EVENT_MANAGER_ROLES),
  asyncHandler((req, res) => eventsWorkflowController.organizerDashboard(req, res))
);
router.post(
  '/predict-draft',
  authorizeCapability(EVENT_MANAGER_ROLES),
  validateRequest(predictDraftSchema),
  asyncHandler((req, res) => eventsWorkflowController.predictDraft(req, res))
);
router.get(
  '/:id/analytics',
  authorizeCapability(EVENT_MANAGER_ROLES),
  asyncHandler((req, res) => eventsWorkflowController.analytics(req, res))
);
router.post(
  '/:id/predict-success',
  authorizeCapability(EVENT_MANAGER_ROLES),
  validateRequest(predictSuccessSchema),
  asyncHandler((req, res) => eventsWorkflowController.predictSuccess(req, res))
);
