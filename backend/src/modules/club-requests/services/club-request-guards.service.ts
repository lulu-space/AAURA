...
export const CLUB_REQUEST_COOLDOWN_DAYS = 60;
export const MIN_CLUB_FOUNDER_ACADEMIC_YEAR = 2;

export type ClubRequestEligibility = {
  eligible: boolean;
  reasons: string[];
...
export class ClubRequestGuardsService {
  async checkEligibility(requesterId: string): Promise<ClubRequestEligibility> {
    ...
    if (cooldownUntil) {
      reasons.push(
        `You can submit again after ${cooldownUntil.toLocaleDateString('en-GB', {
          day: 'numeric',
          month: 'short',
          year: 'numeric'
        })}.`
    ...
    const year = student?.academic_year as number | null | undefined;
  ...
  async assertCanSubmit(requesterId: string, proposedName: string) {
  ...
  async assertAdvisorDeanExists(advisorEmail: string) {
    ...
    if (!data) {
      throw new ApiError(
        400,
        'No dean account exists for that advisor email. Ask your faculty dean to sign up first.'
      );
    }
    if (data.role !== 'dean_of_faculty') {
      throw new ApiError(
        400,
        'Faculty advisor email must belong to a dean of faculty account.'
      );
    }
  ...
  async assertNameAvailable(proposedName: string, excludeRequestId?: string) {
    ...
    if (existingClub) {
      throw new ApiError(
        409,
        `A club named "${existingClub.name}" already exists. Choose a different name.`
      );
    }
    ...
    if (pendingRequest) {
      throw new ApiError(
        409,
        `A pending request for "${pendingRequest.proposed_name}" already exists. Choose a different name.`
      );
    }
  }

  private async cooldownUntil(requesterId: string): Promise<Date | null> {
    ...
    const reviewedAt = lastRejected?.reviewed_at as string | undefined;
