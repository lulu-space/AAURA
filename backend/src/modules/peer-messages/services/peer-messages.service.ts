...
export class PeerMessagesService {
  ...
  private async senderDisplayName(userId: string) {
    const { data, error } = await supabaseAdmin
      .from('users')
    ...
    if (error) throw new ApiError(500, 'Failed to resolve sender name.', error);
  ...
  private peerIdFor(userId: string, message: MessageRow) {
  ...
  async listInbox(userId: string) {
    ...
    if (error) throw new ApiError(500, 'Failed to fetch inbox.', error);
    ...
    if (readsError) throw new ApiError(500, 'Failed to fetch read state.', readsError);

    const readMap = new Map<string, string>();
    for (const row of reads ?? []) {
      readMap.set(row.peer_user_id as string, row.last_read_at as string);
    }

    const grouped = new Map<string, MessageRow[]>();
    for (const row of (messages ?? []) as MessageRow[]) {
      const peerId = this.peerIdFor(userId, row);
      if (!connectedIds.has(peerId)) continue;
    ...
    if (peersError) throw new ApiError(500, 'Failed to fetch peer profiles.', peersError);

    const peerMap = new Map<string, Record<string, unknown>>();
    for (const peer of peers ?? []) {
      peerMap.set(peer.id as string, peer as Record<string, unknown>);
    }
    ...
    const inbox = peerIds.map((peerId) => {
      ...
      const unreadCount = thread.filter(
        (message) =>
          ...
          Date.parse(message.created_at) > lastReadMs
      ).length;

      const peer = peerMap.get(peerId);
      const students = peer?.students as { major?: string; academic_year?: number } | null;
      const email = (peer?.email as string | null) ?? '';
      const fullName = (peer?.full_name as string | null)?.trim();
  ...
  async listConversation(userId: string, otherUserId: string) {
  ...
  async markConversationRead(userId: string, otherUserId: string) {
    ...
    if (error) throw new ApiError(500, 'Failed to mark conversation read.', error);
  ...
  async send(userId: string, recipientUserId: string, body: string) {
    ...
    if (error) throw new ApiError(500, 'Failed to send message.', error);
