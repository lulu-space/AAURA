...
import type { ProvisionUserDto } from '../dto/auth.dto.js';

import {
  assertValidCampusEmail,
  roleFromCampusEmail
} from '../../../shared/utils/campus-email.js';



function normalizeProvisionRpcRow(data: unknown): Record<string, unknown> {
...
export class AuthService {
  ...
  private async attachStudentPayload(

    authUserId: string,

    payload: ProvisionUserDto,
  ...
  private async provisionUserDirect(
    ...
    return this.attachStudentPayload(authUserId, payload, data as Record<string, unknown>);
  ...
  async provisionUser(
    ...
    if (error) {
      ...
      console.warn(
        ...
        '[auth] provision_application_user RPC failed; using direct upsert:',
  ...
  async ensureApplicationUser(authUserId: string, authEmail: string | undefined) {
