...
function baselineProgress(confidence: number) {
  const normalized = confidence > 1 ? confidence / 100 : confidence;
  return Math.min(0.45, Math.max(0.12, normalized * 0.55));
}

export function normalizeStrengths(raw: unknown, confidence = 0.5): StrengthEntry[] {
  ...
  return raw
    ...
    .filter((entry): entry is StrengthEntry => entry != null);
}

export function strengthsFromTraits(
...
export function strengthsFromSkillNames(
  skillNames: string[],
  confidence: number
): StrengthEntry[] {
  const base = baselineProgress(confidence);
...
export function mergeStrengths(
  existing: unknown,
  incoming: StrengthEntry[],
  confidence = 0.5
): StrengthEntry[] {
  const prior = normalizeStrengths(existing, confidence);
  const byName = new Map<string, StrengthEntry>();
...
export function bumpStrengths(
  strengths: StrengthEntry[],
  delta: number,
  changeLabel: string
): StrengthEntry[] {
  if (strengths.length === 0 || delta <= 0) return strengths;
...
export async function bumpStudentSkillProgress(
  userId: string,
  delta: number,
  changeLabel: string
) {
  ...
  if (error) throw new ApiError(500, 'Failed to load student profile.', error);
  ...
  if (updateError) throw new ApiError(500, 'Failed to update skill progress.', updateError);
  return updated;
}
