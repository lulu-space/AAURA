...
function mergeGoals(extracted, interests, strengths) {
 ...
 */
function buildStarterGoals(interests, strengths) {
    const goals = [];
    ...
    if (strengths.length > 0) {
        ...
        const label = typeof first === 'string' ? first : first.name ?? 'campus skills';
...
export class ProfilingWorkflowService {
    async chatWithShams(userId, message) {
    ...
    async getMyDraft(userId) {
        const { data, error } = await supabaseAdmin
            ...
            .maybeSingle();
        if (error)
            throw new ApiError(500, 'Failed to fetch profile draft.', error);
        return data;
    }
    async confirmDraft(userId) {
        const { data: draft, error: draftError } = await supabaseAdmin
            ...
            .maybeSingle();
        if (draftError)
            throw new ApiError(500, 'Failed to load profile draft.', draftError);
        if (!draft)
            throw new ApiError(404, 'No profile draft to confirm. Chat with Shams first.');
        const traitsObj = (draft.traits ?? {});
        ...
        const { data: existingProfile } = await supabaseAdmin
            ...
            .eq('user_id', userId)
            .maybeSingle();
        const strengths = mergeStrengths(existingProfile?.strengths, mergedShamsStrengths, existingProfile?.confidence ?? confidence);
        const mergedInterests = mergeUniqueStrings(existingProfile?.interests, interests);
        const profileSummary = traitsObj.profile_summary ??
        ...
        const academicYear = typeof traitsObj.academic_year === 'number'
            ? traitsObj.academic_year
            : null;
        if (major || academicYear) {
            const studentPatch = {};
            if (major)
                studentPatch.major = major;
            if (academicYear)
                studentPatch.academic_year = academicYear;
    ...
    async regenerateDraft(userId) {
        const { error } = await supabaseAdmin
            .from('student_profile_drafts')
            .delete()
            .eq('user_id', userId);
        if (error)
            throw new ApiError(500, 'Failed to clear profile draft.', error);
        return {
