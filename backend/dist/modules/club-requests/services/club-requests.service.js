...
const SELECT_WITH_REQUESTER = '*, requester:users!club_requests_requester_id_fkey(full_name, email)';
function nameFromRequester(row) {
    const joined = row.requester?.full_name?.trim();
    return joined ? joined : null;
}
function emailFromRequester(row) {
    const joined = row.requester?.email?.trim();
    return joined ? joined : null;
}
async function flattenRequester(rows) {
    ...
    if (error || !users)
        return flattened;
    const byId = new Map(users.map((user) => [
        user.id,
        {
...
export class ClubRequestsService {
    ...
    async create(requesterId, payload) {
        ...
        if (error) {
            if (error.code === '23505') {
                throw new ApiError(409, 'A club or pending request with this name already exists. Choose a different name.');
            }
            ...
            if (error.code === '42703' ||
                ...
                throw new ApiError(503, 'Club request storage is not configured on the server. Apply database migration 0021_club_request_guards.sql.');
            }
    ...
    async listMine(requesterId) {
        ...
        if (error)
            throw new ApiError(500, 'Failed to fetch your club requests.', error);
        return data;
    }
    ...
    async listWithRequester(pendingOnly) {
        ...
        return await flattenRequester(data);
    }
    async loadPendingRequest(requestId) {
        const { data: request, error } = await supabaseAdmin
            ...
            .maybeSingle();
        if (error)
            throw new ApiError(500, 'Failed to load club request.', error);
        if (!request)
            throw new ApiError(404, 'Club request not found.');
     ...
     */
    async approve(requestId, reviewerId, reviewNote) {
        const request = await this.loadPendingRequest(requestId);
        await clubRequestGuardsService.assertNameAvailable(request.proposed_name, requestId);
        ...
        if (clubError || !club) {
            ...
            throw new ApiError(409, 'Could not create the club (the name may already be taken).', clubError);
        }
        ...
        const { data: updated, error: updateError } = await supabaseAdmin
            ...
            .single();
        if (updateError)
            throw new ApiError(500, 'Failed to finalize approval.', updateError);
    ...
    async reject(requestId, reviewerId, reviewNote) {
        const request = await this.loadPendingRequest(requestId);
        const { data: updated, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new ApiError(500, 'Failed to reject club request.', error);
        await supabaseAdmin.from('notifications').insert({
            user_id: request.requester_id,
     ...
     */
    async revoke(requestId, reviewerId, reviewNote) {
        const { data: request, error } = await supabaseAdmin
            .from('club_requests')
            .select('*')
            .eq('id', requestId)
