...
export class EventsController extends CrudController {
    ...
    async list(req, res) {
        const data = await eventsMutationService.listForUser(req.authUser.id, req.authUser?.role);
        res.json({ message: 'Events fetched successfully.', data });
    }
    async create(req, res) {
        const data = await eventsMutationService.create(req.authUser.id, req.authUser?.role, req.body);
        res.status(201).json({ message: 'Events created successfully.', data });
    }
    async update(req, res) {
        const data = await eventsMutationService.update(req.params.id, req.authUser.id, req.authUser?.role, req.body);
