...
const SELECT_WITH_STUDENT = '*, student:users!volunteering_records_user_id_fkey(full_name)';
function nameFromJoin(row) {
...
async function attachStudentNames(rows) {
    ...
    return withJoin.map((row) => row.student_name
        ? row
        : { ...row, student_name: byId.get(row.user_id) ?? null });
}
export class VolunteeringWorkflowService {
    async listPending() {
        ...
        if (error)
            throw new ApiError(500, 'Failed to fetch pending records.', error);
    ...
    async approve(recordId, staffUserId, approvalNote) {
        return this.setStatus(recordId, staffUserId, 'approved', approvalNote);
    }
    async reject(recordId, staffUserId, approvalNote) {
        return this.setStatus(recordId, staffUserId, 'rejected', approvalNote);
    }
    async withdraw(recordId, staffUserId) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new ApiError(500, 'Failed to withdraw volunteering decision.', error);
        const [withName] = await attachStudentNames([data]);
        if (data.user_id) {
    ...
    async setStatus(recordId, staffUserId, status, approvalNote) {
        const { data: existing, error: fetchError } = await supabaseAdmin
            .from('volunteering_records')
            .select('id, status, user_id, hours, title')
            .eq('id', recordId)
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new ApiError(500, 'Failed to update volunteering record.', error);
        const [withName] = await attachStudentNames([data]);
        if (status === 'approved') {
            const hours = Number(data.hours ?? 0);
            const title = String(data.title ?? '');
            const isEnrollment = hours === 0 || title.toLowerCase().startsWith('enrollment:');
            if (!isEnrollment && hours > 0 && data.user_id) {
                const delta = Math.min(0.2, hours * 0.02);
                await bumpStudentSkillProgress(data.user_id, delta, `+${Math.round(delta * 100)}% from volunteer hours`).catch(() => null);
            }
        }
        if (data.user_id) {
            const hours = Number(data.hours ?? 0);
