...
import { FACULTY_OPTIONS, filterClubsForFaculty, filterEventsForFaculty, listStudentUserIdsInFaculty, majorsForFaculty, requireDeanFaculty } from '../../../shared/utils/faculty-scope.js';
...
export class DeanService {
    ...
    async getDashboard(userId, role) {
        ...
        const facultyEvents = await filterEventsForFaculty((eventsRaw ?? []), faculty, userId);
        ...
        const facultyClubs = await filterClubsForFaculty((clubsRaw ?? []), faculty);
        ...
        const approvedVolunteerHours = (approvedVolunteerRows ?? []).reduce((sum, row) => sum + Number(row.hours ?? 0), 0);
        const { data: reservations, error: reservationError } = await supabaseAdmin
            .from('event_reservations')
            .select('id, event_id, reservation_status')
            .in('event_id', facultyEvents.map((event) => event.id).filter(Boolean));
        if (reservationError) {
            throw new ApiError(500, 'Failed to load engagement stats.', reservationError);
        }
        const checkedIn = reservations?.filter((row) => row.reservation_status === 'checked_in').length ?? 0;
        const enrolled = reservations?.filter((row) => row.reservation_status !== 'cancelled').length ?? 0;
        const pendingReviews = facultyEvents.filter((event) => event.is_approved === false && event.status !== 'cancelled').length;
    ...
    async listEvents(userId, role) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .order('starts_at', { ascending: false });
        if (error)
            throw new ApiError(500, 'Failed to load events.', error);
        return filterEventsForFaculty((data ?? []), faculty, userId);
    }
    async listClubs(userId, role) {
        const faculty = await requireDeanFaculty(userId, role);
        const { data, error } = await supabaseAdmin.from('clubs').select('*').order('name');
        if (error)
            throw new ApiError(500, 'Failed to load clubs.', error);
        return filterClubsForFaculty((data ?? []), faculty);
    }
    async getInsights(userId, role) {
        const faculty = await requireDeanFaculty(userId, role);
        const studentIds = await listStudentUserIdsInFaculty(faculty);
        ...
        const engagementPredictions = events
            ...
            .sort((a, b) => (Number(b.predicted_success) || 0) - (Number(a.predicted_success) || 0))
            .slice(0, 8);
        ...
        if (studentIds.length > 0) {
            ...
            if (error)
                throw new ApiError(500, 'Failed to load student interests.', error);
    ...
    async generateReport(userId, role, type) {
        const faculty = await requireDeanFaculty(userId, role);
        ...
        switch (type) {
            case 'events': {
                ...
                return {
                    ...
                    summary: {
                        ...
                        pending_review: events.filter((event) => event.is_approved === false && event.status !== 'cancelled').length
                    },
            ...
            case 'volunteering': {
                ...
                if (error)
                    throw new ApiError(500, 'Failed to load volunteering report.', error);
            ...
            case 'engagement': {
                ...
                if (error)
                    throw new ApiError(500, 'Failed to load engagement report.', error);
                ...
                return {
                    ...
                    summary: {
                        ...
                        total_enrollments: reservations?.filter((row) => row.reservation_status !== 'cancelled')
                            .length ?? 0,
                        total_check_ins: reservations?.filter((row) => row.reservation_status === 'checked_in').length ?? 0
                    },
    ...
    async listAnnouncements(userId, role) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .limit(30);
        if (error)
            throw new ApiError(500, 'Failed to load announcements.', error);
        return data ?? [];
    }
    async sendAnnouncement(userId, role, payload) {
        const faculty = await requireDeanFaculty(userId, role);
        const studentIds = await listStudentUserIdsInFaculty(faculty);
        ...
        if (studentIds.length > 0) {
            ...
            if (error)
                throw new ApiError(500, 'Failed to send announcement.', error);
            sent = rows.length;
        }
