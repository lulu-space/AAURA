...
export class AdminController {
    ...
    async updateUser(req, res) {
        const data = await adminService.updateUser(req.authUser.id, req.params.id, req.body);
        res.json({ message: 'User updated successfully.', data });
    }
    async content(_req, res) {
    ...
    async moderateEvent(req, res) {
        const data = await adminService.moderateEvent(req.authUser.id, req.params.id, req.body.action);
        res.json({ message: 'Event moderated successfully.', data });
    }
    async moderateClub(req, res) {
        const data = await adminService.moderateClub(req.authUser.id, req.params.id, req.body.action);
        res.json({ message: 'Club moderated successfully.', data });
    }
    async moderatePost(req, res) {
        const data = await adminService.moderatePost(req.authUser.id, req.params.id, req.body.hidden);
        res.json({ message: 'Post moderated successfully.', data });
    }
    async moderateMessage(req, res) {
        const data = await adminService.moderateMessage(req.authUser.id, req.params.id, req.body.hidden);
    ...
    async updateSettings(req, res) {
        const data = await adminService.updateSettings(req.authUser.id, req.params.key, req.body.value);
        res.json({ message: 'Settings updated successfully.', data });
    }
    async badges(_req, res) {
        const data = await adminService.listBadges();
        res.json({ message: 'Badges fetched successfully.', data });
    }
    async updateBadge(req, res) {
        const data = await adminService.updateBadge(req.authUser.id, req.params.id, req.body);
