...
export class SupabaseCrudService {
    ...
    selectClause;
    ownership;
    constructor(tableName, selectClause = '*', ownership = {}) {
        this.tableName = tableName;
        this.selectClause = selectClause;
        this.ownership = ownership;
    }
    async assertCanMutate(id, userId, role) {
        if (role === 'admin')
            return;
    ...
    async list(userId, role) {
        ...
        if (error)
            throw new ApiError(500, `Failed to fetch ${this.tableName}.`, error);
        return data;
    }
    ...
    async create(userId, role, payload) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new ApiError(500, `Failed to create ${this.tableName}.`, error);
        return data;
    }
    async update(id, userId, role, payload) {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .single();
        if (error)
            throw new ApiError(500, `Failed to update ${this.tableName}.`, error);
        return data;
    }
    async remove(id, userId, role) {
        ...
        if (error)
            throw new ApiError(500, `Failed to delete ${this.tableName}.`, error);
        return { id };
    }
}
