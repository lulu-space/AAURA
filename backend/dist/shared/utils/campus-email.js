...
export const CAMPUS_EMAIL_INVALID_MESSAGE = 'Use an AAUP campus email only: @student.aaup.edu (students), @staff.aaup.edu (Student Affairs), @aaup.edu (Dean of Faculty), or admin@aaup.edu (admin).';
...
export function parseCampusEmail(email) {
...
export function validateCampusEmail(email) {
    const parsed = parseCampusEmail(email);
    if (!parsed)
        return null;
    if (!ALLOWED_DOMAINS.has(parsed.domain))
        return null;
    if (parsed.domain === 'student.aaup.edu')
        return 'student';
    if (parsed.domain === 'staff.aaup.edu')
...
export function assertValidCampusEmail(email) {
...
export function roleFromCampusEmail(email) {
