-- Reject any email outside the four AAUP campus patterns at provisioning time.
...
begin
  ...
  v_is_campus :=
    v_email like '%@student.aaup.edu'
    or v_email like '%@staff.aaup.edu'
    or (
      v_email like '%@aaup.edu'
      and v_email not like '%@student.aaup.edu'
      and v_email not like '%@staff.aaup.edu'
    );

  if not v_is_campus or position('@' in v_email) <= 1 then
    raise exception
      'Invalid campus email. Use @student.aaup.edu, @staff.aaup.edu, @aaup.edu (Dean), or admin@aaup.edu (admin).';
  end if;
